...
For all $k = 0, \ldots, K-1$, we first choose $\xi_k^{(0)} = \mathbf{0}_d$ and ${\gamma_{k,a}^l}^{(0)} = 0$ for all $l=1, \ldots, L$ and $a=1, \ldots, \cA$. Then, we initialize $\lambda_0^{(0)}$ and $\gamma_{k,0}^{(0)}$ like if there is no subgroup ($\gamma_{0,0}^{(0)} = \cdots = \gamma_{K-1,0}^{(0)}$) with a standard Cox proportional hazards regression with time-independent features using \texttt{R} package \texttt{survival}. Finally, the longitudinal submodels parameters $\beta_k^{(0)}$, $D^{(0)}$ and $\phi^{(0)}$ are initialized -- again like if there is no subgroup ($\beta_0^{(0)} = \cdots = \beta_{K-1}^{(0)}$) -- using a multivariate linear mixed model with an explicit EM algorithm, being itself initialized with univariates fits using \texttt{R} package \texttt{nlme}.
...
We detail in this section the metric considered to evaluate risk prediction performances. Let us denote by $M$ the marker under study (e.g., the marker~\eqref{eq:marker-def} for the lights model), and assume that it is measured once at $t = t^{max}$.
...
In order to assess the proposed method, we run an extensive Monte Carlo simulation study in the case $K=2$ (see Section~\ref{sec:K=2}). We first generate $G_i \sim \cB\big(\pi_1\big)$, with $\pi_1 \in [0,1]$ a desired high-risk subjects proportion and where $\cB(\alpha)$ denotes the Bernoulli distribution with parameter $\alpha \in [0,1]$.
We then choose a coefficient vector 
\begin{equation}
  \label{eq:xi-sparse}
...
with $\bSigma_1(\rho_1)$ a $(p \times p)$ Toeplitz covariance matrix \citep{mukherjee1988some} with correlation $\rho_1 \in (0, 1)$. 
We then add a $gap \in \R^+$ value for high-risk subjects and subtract it for low-risk subjects, only on active features, that is
\begin{center}
$x_{ij} \leftarrow x_{ij} + (-1)^{\ind{\{G_i=0\}}} gap\ \text{for } j = 1, \dots, s$.
\end{center}
Note that this is equivalent to generate the time-independent features according to a gaussian mixture.
...
for $l=1, \ldots, L$. $\Upsilon_{l} \geq 0$ is called baseline intensity, and corresponds to the exogenous probability of having a measurement for process $l$. We sample $\Upsilon_{l} \sim \cU\big([0.1,0.5]\big)$ ] which produces unbalanced baselines, where $\cU\big([a,b]\big)$ stands for the uniform distribution on a segment $[a,b]$.
The matrix $A = [A_{ll'}]_{1 \leq l,l' \leq L}$ is the adjacency matrix such that $A_{ll'} \geq 0$ quantifies the impact of past measurement time of process $l'$ on the measurement time of process $l$, and $\upsilon \geq 0$ is a memory parameter. We sample $A_{ll'} \sim \cU\big([0.05,0.1]\big)$, but then enforce $A$ to be sparse (with a chosen density of $0.3$) by randomly zeroing non-diagonal coefficients. Simulation of the Hawkes processes
...
\begin{equation}
  ...
  1 - t_i^{max} \sim T_i \times \text{Beta}(\nu_1, \nu_2)
\end{equation}
...
$\begin{pmatrix}
  1 \\ 0.5
\end{pmatrix}$ 
& 0.5 & 0.5 & 3 & (2, 5) & $(10^{-3}, 0.1)$ & (1, 0.1) & 0.3 & 0.4 & 0.1 \\
\bottomrule
\end{tabular}}
\end{table}
\begin{figure}[!htb]
...
\section*{Software}
All the methodology discussed in this paper is implemented in \texttt{Python}. The code is available from \href{https://github.com/Califrais/lights}%
{https://github.com/Califrais/lights} in the form of annotated programs, together with a notebook tutorial.
...
We re-write the conditional distribution of $y_i|b_i; \theta $ is in form
\begin{align*}
	f(y_i|b_i, \theta) &= \exp \big\{(y_i \odot \Phi_i)^\top M_{i} - c_\phi(M_{i}) + d_\phi(y_i) \big\} \\
	&= \exp \big\{( \Sigma_iy_i)^\top M_{i} - c_\phi(M_{i}) + d_\phi(y_i) \big\}
\end{align*}
where $\Sigma_i$ is a diagonal matrix whose diagonal is $\Phi_i$. And if the distribution $b_i; \theta$ is Gaussian, then its density form is
...
\begin{align*}
  ...
  &= \text{log} (f(y, b ; \theta) \\ 
  &= \sum_{i=1}^n \text{log}(f(y_i|b_i, \theta)) + \text{log}(f(b_i ; \theta))\\
  &= \sum_{i=1}^n (\Sigma_iy_i)^\top M_{i} - c_\phi(M_{i}) + d_\phi(y_i) \\
  &  \quad -\dfrac12 (r \log 2\pi + \log|D| + b_i^\top D^{-1}b_i).
\end{align*}
...
\begin{equation*}
  \cQ_n(\theta, \theta^{(w)}) = \E_{\theta^{(w)}}[\ell_n^\text{comp}(\theta) | \cD_n].
\end{equation*}
Here, computing $\E_{\theta^{(w)}}[\ell_n^\text{comp}(\theta) | \cD_n]$  reduces to computing $E_{\theta^{(w)}}[b_i | y_i, \theta]$ and $E_{\theta^{(w)}}[b_ib_i^T | y_i, \theta]$ for $i = 1,..,n$\\
\\
If the marginal distributions of $y_i$ and $b_i$ are both Gaussian, then conditional distribution of $b_i$ is also Gaussian with a mean and a variance that can be computed. Indeed, from Bayes Theorem,
\begin{align*}
    f(b_i|y_i; \theta) &= \frac{f(y_i|b_i; \theta)f(b_i; \theta)}{f(y_i; \theta} \\
    &= C_1 \text{ x } \exp \big\{( S_iy_i)^\top M_{i} - c_\phi(M_{i}) + d_\phi(y_i) \big\} \text{ x } \exp \Big\{ -\dfrac12 b_i^\top D^{-1}b_i \Big\}\\
    &= C_2  \text{ x } \exp \big\{-\dfrac12 {b_i}^\top ({V_i}^\top \Sigma_iV_i)b_i + {b_i}^\top {V_i}^\top S(y_i - U_i\beta) \big\} \text{ x } \exp \Big\{ -\dfrac12 b_i^\top D^{-1}b_i \Big\}\\
    &= C_2  \text{ x } \exp \big\{-\dfrac12 {b_i}^\top ({V_i}^\top \Sigma_iV_i + D^{-1})b_i + {b_i}^\top {V_i}^\top \Sigma_i(y_i - U_i\beta) \big\}\\
    &= C_2 \text{ x } \exp \big\{ -\dfrac12 (b_i - \mu_i)^\top \Omega_i^{-1} (b_i - \mu_i) \big\}
\end{align*}
where,
\begin{align*}
    \Omega_i = ({V_i}^\top \Sigma_iV_i + D^{-1})^{-1} \text{  ;   } \mu_i = \Omega_i{V_i}^\top \Sigma_i(y_i - U_i\beta)
\end{align*}
Then,
\begin{align*}
    \E_{\theta^{(w)}}[b_i | y_i, \theta] &= \mu_i \\
    \E_{\theta^{(w)}}[b_ib_i^T | y_i, \theta] &= \text{Var}_{\theta^{(w)}}[b_i | y_i, \theta] + \E_{\theta^{(w)}}[b_i | y_i, \theta]\E_{\theta^{(w)}}[b_i | y_i, \theta]^\top\\
                                        &= \Omega_i + \mu_i\mu_i^\top
\end{align*}
...
\begin{align*}
  \label{eq:grad-beta}
  \frac{\partial \cQ_n(\theta, \theta^{(w)})(\beta)}{\partial \beta} &= \sum_{i=1}^n 
  \Big[ {y_i}^\top \Sigma U_{i} - U_{i}^\top \Sigma V_{i}\hat \E_{\theta^{(w)}}[ b_i | y_i; \theta]) - U_{i}^\top \Sigma U_{i}\beta \Big] \\
  \frac{\partial \cQ_n(\theta, \theta^{(w)})(\phi)}{\partial \phi^l} &= \sum_{i=1}^n 
  \dfrac12 {\phi_l}^{-2}\Big[ (y_i^l - U_{il}\beta_l)^\top (y_i^l - U_{il}\beta_l - 2V_{il}\hat \E_{\theta^{(w)}}[ b_i^l | y_i^l; \theta])) \\
  & \quad \quad +  \Tr\big(V_{il}^\top V_{il}\hat \E_{\theta^{(w)}}[ b_i^l {b_i^l}^\top | y_i^l; \theta]\big) \Big] - \frac{n_i^l}{2} {\phi_l}^{-1}\\
  \frac{\partial \cQ_n(\theta, \theta^{(w)})(D)}{\partial D} &= \sum_{i=1}^n 
   \dfrac12 \big( D - \E_{\theta^{(w)}}[ b_i {b_i}^\top | y_i; \theta]\big)
\end{align*}
\\
Then, the updated parameter $\theta^{(w)}$ will be
\begin{align*}
  \beta^{(w+1)} &= \Big(\sum_{i=1}^n U_{i}^\top U_{i}\Big)^{-1}  \Big( \sum_{i=1}^n \big[ U_{i}^\top  y_i - U_{i} V_{i}\hat \E_{\theta^{(w)}}[ b_i | y_i; \theta]) \big] \Big)\\
  \phi_l^{(w+1)} &= \big(\sum_{i=1}^n n_i^l \big)^{-1}  \sum_{i=1}^n \Big[ (y_i^l - U_{il}\beta_l^{(w+1)})^\top (y_i^l - U_{il}\beta_l^{(w+1)} - 2V_{il}\hat \E_{\theta^{(w)}}[ b_i^l | y_i^l; \theta])) \\
  & \hspace{3cm}+ \Tr\big(V_{il}^\top V_{il}\hat \E_{\theta^{(w)}}[ b_i^l {b_i^l}^\top | y_i^l; \theta]\big) \Big] \\
  D^{(w+1)} &= n^{-1} \sum_{i=1}^n \E_{\theta^{(w)}}[ b_i {b_i}^\top | y_i; \theta]
\end{align*}

...
\[ P^{(w)}_{n}(\xi_0, \ldots, \xi_K) = -n^{-1} \sum_{i=1}^n \sum_{k=0}^{K-1} \hat \pi_{ik}^{\theta^{(w)}} \log \pi_{\xi_k}(x_i) \]
based on the quantities involved in $\cQ_n(\theta, \theta^{(w)})$ that depend on all of $\xi_k$ for $k=0, \ldots, K-1$. Then we can also write
\[ P^{(w)}_{n, k}(\xi_k) = -n^{-1} \sum_{i=1}^n \Big[\hat \pi_{ik}^{\theta^{(w)}} x_i^\top \xi_k - \log \sum_{k^{\prime}=0}^{K-1} e^{x_i^\top\xi_{k^{\prime}}}\Big] \]
based on the quantities involved in $P^{(w)}_{n}(\xi_0, \ldots, \xi_K)$ that depend on only one $\xi_k$. The update for each $\xi_k^{(w)}$ therefore requires to solve the following convex minimization problem
\begin{equation}

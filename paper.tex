...
\begin{align*}
  ...
  & - \sum_{j=1}^J \lambda_0^{(w)}(t_j) \hat \E_{\theta^{(w)}}[\exp \big\{\gamma_k^\top \Psi_i(t_j)\big\} | t_i, \delta_i, y_i] \ind{\{t_j\leq t_i\}} \Big]
\end{align*}
...
\begin{align}
  ...
  &\frac{\partial Q_{n,k}^{(w)}(\gamma_k)}{\partial \gamma_{k,a}^l} = -n^{-1} \sum_{i=1}^n \hat \pi_{ik}^{\theta^{(w)}} \Big[ \delta_i  \hat \E_{\theta^{(w)}}[ \varphi_a(t_i, b_i^l) | t_i, \delta_i, y_i] \\ \nonumber
   &\qquad\qquad - \sum_{j=1}^J \lambda_0^{(w)}(t_j) \hat \E_{\theta^{(w)}}[ \varphi_a(t_j, b_i^l) \tilde g^1_{t_j, \gamma_k}(b_i) | t_i, \delta_i, y_i] \ind{\{t_j\leq t_i\}} \Big]
\end{align}
...
\begin{equation}
  ...
  \lambda_0^{(w+1)}(t)= \dfrac{\sum_{i=1}^n \delta_i \ind{\{t=t_i\}}}{\sum_{i=1}^n \sum_{k=0}^{K-1} \hat \pi_{ik}^{\theta^{(w)}} \hat \E_{\theta^{(w)}}\Big[ \tilde g^1_{t, \gamma_k^{(w+1)}}(b_i) | t_i, \delta_i, y_i \Big] \ind{\{t_i \geq t\}} },
\end{equation}
...
\begin{equation}
  ...
  \left\{
    ...
    \tilde g^5_{s, \gamma_k}&: b_i \mapsto b_i g^4_{s, \gamma_k}(b_i), \\
    \tilde g^6_{s, \gamma_k}&: b_i \mapsto g^3_{s}(b_i)^\top g^4_{s}(b_i).
    \end{array}
  \right. 
\end{equation}

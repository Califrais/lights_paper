...
\begin{equation}
  ...
  {\beta_k^l}^{(w+1)}= \dfrac{\hat \pi_{ik}^{\theta^{(w)}} \Big[U_{il}^\top I_{n_{il}} (y_i^l - V_{il}\hat \E_{\theta^{(w)}}[ b_i | t_i, \delta_i, y_i])\Big]}{\sum_{i=1}^n \hat \pi_{ik}^{\theta^{(w)}} \Big[U_{il}^\top I_{n_{il}} U_{il} \Big]}.
\end{equation}

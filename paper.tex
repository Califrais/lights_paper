...
or the updated baseline hazard function~\eqref{eq:update_lambda_0} can be rewrote as
\begin{equation*}
  \label{eq:update_lambda_0_matrix}
  \lambda_0^{(w+1)}(t)= \dfrac{\underset{n}{\text{sum }} \boldsymbol{\delta} \cI_1 }{\underset{n}{\text{sum }} \underset{K = 2}{\text{sum }} \hat \E_{\theta^{(w)}}[\boldsymbol{g^4}] \cI_2 \boldsymbol{\hat \pi^{\theta^{(w)}}}  },
\end{equation*}
where 
\begin{equation*}
  \left\{
    ...
    \boldsymbol{\hat \pi^{\theta^{(w)}}} \in \R^{2 \times n} \text{ with }  \boldsymbol{\hat \pi_1^{\theta^{(w)}}}= (\hat \pi_{11}^{\theta^{(w)}}, \ldots, \hat \pi_{n1}^{\theta^{(w)}})^\top \in \R^n \text{ and } \boldsymbol{\hat\pi_0^{\theta^{(w)}}} = 1 - \boldsymbol{\hat\pi_1^{\theta^{(w)}}}.
    \end{array}
  \right. 
\end{equation*}

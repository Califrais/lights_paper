...
\begin{equation}
  ...
  \Lambda_{ik,\tilde g_i}^{\theta^{(w)}} = \int_{\R^r} \tilde g_i(b_i) f(t_i, \delta_i | b_i, G_i = k ; \theta^{(w)}) f(b_i | y_i, G_i = k; \theta^{(w)}) \dd b_i,
\end{equation}
...
{ 
  ...
  Compute $\breve{b}_{\breve{n}} = C_i^{(w)} \Omega_{\breve{n}} \in \R^r$ \\
...
$\text{M}_i= V_i^\top{\Sigma_i^{(w)}}^{-1}V_i + {D^{(w)}}^{-1}$ or $\text{M}_i= D^{(w)}$

\texttt{@VT: weird that $D^{(w)}$ and ${D^{(w)}}^{-1}$ ?}
...
\label{alg:ISTA}
\KwInput{A vector $v$, a function $f$ (its gradient), hyper-parameters $\tilde \eta$ and $\zeta$}
...
{ 
  \vspace{-.8cm}
  \begin{align*}
  \flat^{(\breve{w}+1)} \in &\argmin_u \Big\{\dfrac{1}{2\varrho_{\breve{w}}} \left\lVert u - \big(\flat^{(\breve{w})} - \varrho_{\breve{w}} \dfrac{\partial f(u)}{\partial u} \big)\right\rVert_2^2 + \zeta \norm{u}_{\text{sg} l_1, \tilde{\eta}} \Big\} \\
  &\coloneqq\text{prox}_{\text{sg} l_1, \tilde{\eta}, \zeta} \Big(\flat^{(\breve{w})} - \varrho_{\breve{w}} \dfrac{\partial f(u)}{\partial u} \Big)
  \end{align*}
  \vspace{-.3cm}
}
...
\vspace{.5cm}
TODO w/ VT: note on $\breve{w}^{max}$. Lemma~\ref{lemma:prox-sgl1} bellow states that one can easily compute the proximal operator of the sparse group lasso defined in Algorithm~\ref{alg:ISTA}.
\begin{lemma}
...
\begin{equation}
  ...
  \gamma_k^{(w+1)} \in \argmin_{\gamma_k \in \R^{L\mathcal{A}}} Q^{(w)}_{n,k}({\gamma_k^0}^{(w)}, \gamma_k) + \zeta_{3,k} \norm{\gamma_k}_{\text{sg} l_1, \tilde{\eta}}.
\end{equation}
...
\begin{equation}
  ...
  \gamma_k^{(w+1)} = \text{ISTA}_{\text{sg} l_1}\big(\gamma_k^{(w)}, Q^{(w)}_{n,k}({\gamma_k^0}^{(w)}, \cdot), \tilde \eta, \zeta_{3,k}\big)
\end{equation}
with the gradient being known since
\begin{align}
  \label{eq:grad-Q-2}
  \frac{\partial Q_{n,k}^{(w)}({\gamma_k^0}^{(w)}, \gamma_k)}{\partial \gamma_{k,a}^l} &= -n^{-1} \sum_{i=1}^n \hat \pi_{ik}^{\theta^{(w)}} \Big[ \delta_i  \hat \E_{\theta^{(w)}}[ \varphi_a(t_i, {\beta_k^l}^{(w+1)}, b_i^l) | t_i, \delta_i, y_i] \\ \nonumber
   &- \sum_{j=1}^J \lambda_0^{(w)}(t_j) \hat \E_{\theta^{(w)}}[ \varphi_a(t_j, {\beta_k^l}^{(w+1)}, b_i^l) \tilde g_{i, t_j, {\gamma_k^0}^{(w)}, \gamma_k, \beta_k^{(w+1)}}^1(b_i) | t_i, \delta_i, y_i] \ind{\{t_j\leq t_i\}} \Big]
\end{align}

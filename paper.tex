...
Let us introduce the following useful function
\begin{equation}
  ...
  \right.
\end{equation}
and present the parameters updates in the order given in Algorithm~\ref{alg:QNMCEM}.
...
\end{align}
based on the quantities involved in $\cQ_n(\theta, \theta^{(w)})$ depending on $\beta_k$, with $\tilde g_{t_j, \gamma_k^{(w+1)}, \beta_k}^1$ and $\tilde g_{t_i, \gamma_k^{(w+1)}, \beta_k}^2$ defined in~\eqref{eq:g-tilde-1-2}. Since $R^{(w)}_{n,k}$ depends on parameters estimates at iteration $w+1$ that have not been computed yet ($\gamma_k^{(w+1)}$, $\lambda_0^{(w+1)}$ and $\phi^{(w+1)}$), we denote $R^{(w^-)}_{n,k}$ the function obtained from~\eqref{eq:R-def} by replacing $(w+1)$ with $(w)$, that is using the last iteration estimates.
...
\begin{align*}
  ...
  &+ U_{il}^\top \phi_l^{-2}I_{n_{il}} (y_i^l - U_{il}\beta_k^l - V_{il}\hat \E_{\theta^{(w)}}[ b_i | t_i, \delta_i, y_i])\Big]
\end{align*}
...
\begin{equation*}
  \frac{\partial Q_{n,k}^{(w)}(\gamma_k)}{\partial \gamma_{k,0}} = -n^{-1} \sum_{i=1}^n \hat \pi_{ik}^{\theta^{(w)}} \Big[ \delta_i - \sum_{j=1}^J \lambda_0^{(w+1)}(t_j) \hat \E_{\theta^{(w)}}[ \tilde g_{t_j, \gamma_k, \beta_k^{(w+1)}}^1(b_i) | t_i, \delta_i, y_i] \ind{\{t_j\leq t_i\}} \Big] x_i
\end{equation*}
and
\begin{align*}
  \frac{\partial Q_{n,k}^{(w)}(\gamma_k)}{\partial \gamma_{k,a}^l} = -n^{-1} &\sum_{i=1}^n \hat \pi_{ik}^{\theta^{(w)}} \Big[ \delta_i  \hat \E_{\theta^{(w)}}[ \varphi_a(t_i, {\beta_k^l}^{(w+1)}, b_i^l) | t_i, \delta_i, y_i] \\
   &- \sum_{j=1}^J \lambda_0^{(w+1)}(t_j) \hat \E_{\theta^{(w)}}[ \varphi_a(t_j, {\beta_k^l}^{(w+1)}, b_i^l) \tilde g_{t_j, \gamma_k, \beta_k^{(w+1)}}^1(b_i) | t_i, \delta_i, y_i] \ind{\{t_j\leq t_i\}} \Big]
\end{align*}

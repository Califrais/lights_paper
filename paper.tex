...
\begin{equation}
	...
	\lambda(t|\cM_k(t), G = k) = \lambda_0(t) \exp \Big\{ x^\top \gamma_k^0 + \sum_{l=1}^L \sum_{a=1}^\cA {\gamma_{k,a}^l}^\top \varphi_a(t, b^l) \Big\},
\end{equation}
...
\begin{align*}
  ...
  & \quad + \delta_i \Big(\log \lambda_0(t_i) + x_i^\top \gamma_k^0 + \sum_{l=1}^L \sum_{a=1}^\cA {\gamma_{k,a}^l}^\top \varphi_a(t_i, b_i^l) \Big) \\
  & \quad -\int_0^{t_i} \lambda_0(s) \exp \Big\{ x_i^\top \gamma_k^0 + \sum_{l=1}^L \sum_{a=1}^\cA {\gamma_{k,a}^l}^\top \varphi_a(s, b_i^l) \Big\} \dd s \\
...
\begin{equation}
  ...
  \left\{
    \begin{array}{ll}
        \tilde g_{i, s, \gamma_k^0, \gamma_k}^1&: b_i \mapsto \exp \big\{ x_i^\top \gamma_k^0 + \tilde g_{s, \gamma_k}^2(b_i) \big\}, \\
...
\begin{align*}
  Q^{(w)}_{n,k}(\gamma_k^0, \gamma_k) = -n^{-1} \sum_{i=1}^n \hat \pi_{ik}^{\theta^{(w)}} \Big[ &\delta_i \hat \E_{\theta^{(w)}}[ \log \circ \tilde g_{i, t_i, \gamma_k^0, \gamma_k}^1(b_i) | t_i, \delta_i, y_i] \\
  & - \sum_{j=1}^J \lambda_0^{(w)}(t_j) \hat \E_{\theta^{(w)}}[ \tilde g_{i, t_j, \gamma_k^0, \gamma_k}^1(b_i) | t_i, \delta_i, y_i] \ind{\{t_j\leq t_i\}} \Big]
\end{align*}
based on the quantities involved in $\cQ_n(\theta, \theta^{(w)})$ that depend on $\gamma_k^0$ and $\gamma_k$, with $\tilde g_{i, t_j, \gamma_k^0, \gamma_k}^1$ defined in~\eqref{eq:g-tilde-1-2}. The update for ${\gamma_k^0}^{(w)}$ therefore requires to solve the following minimization problem
\begin{equation}
...
\begin{align}
  ...
  &\times \hat \E_{\theta^{(w)}}[ \tilde g_{i, t_j, \gamma_k^0, {\gamma_k}^{(w)}}^1(b_i) | t_i, \delta_i, y_i] \ind{\{t_j\leq t_i\}} \Big] x_i.
\end{align}
...
\begin{align}
  \label{eq:grad-Q-2}
  &\frac{\partial Q_{n,k}^{(w)}({\gamma_k^0}^{(w+1)}, \gamma_k)}{\partial \gamma_{k,a}^l} = -n^{-1} \sum_{i=1}^n \hat \pi_{ik}^{\theta^{(w)}} \Big[ \delta_i  \hat \E_{\theta^{(w)}}[ \varphi_a(t_i, b_i^l) | t_i, \delta_i, y_i] \\ \nonumber
   &\qquad\qquad - \sum_{j=1}^J \lambda_0^{(w)}(t_j) \hat \E_{\theta^{(w)}}[ \varphi_a(t_j, b_i^l) \tilde g_{i, t_j, {\gamma_k^0}^{(w+1)}, \gamma_k}^1(b_i) | t_i, \delta_i, y_i] \ind{\{t_j\leq t_i\}} \Big]
\end{align}

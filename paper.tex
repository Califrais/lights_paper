...
&\times \exp \Big\{ - \sum_{j=1}^{J-1} (t_{j+1}^u-t_j^u) \lambda_0^{(w)}(t_j^u) g^4_{t_j^u, \gamma_k^{(w)}}(b_i) \ind{\{t_j^u < t_i\}} \Big\}
\end{align*}
so that one can compute the tensor
\[ \underset{J}{\text{sum}} \big( \boldsymbol{g^4_{\gamma_k}} \boldsymbol{\lambda_0^{(w)}} \cI_1 \big)^{\boldsymbol{\delta}} \exp \big\{- \underset{J}{\text{sum}} \big(\boldsymbol{g^4_{\gamma_k}} \boldsymbol{\lambda_0^{(w)}} \boldsymbol{t^u} \cI_2 \big) \big\} \in \R^{??} \]
where 
\begin{equation*}
  \label{eq:g-tilde}
  \left\{
    ...
    \boldsymbol{\lambda_0^{(w)}} = \big(\lambda_0^{(w)}(t_1^u), \ldots, \lambda_0^{(w)}(t_{J-1}^u) \big)^\top \in \R^{J - 1}, \\
    \boldsymbol{\delta} = (\delta_1, \ldots, \delta_n)^\top \in \R^n , \\
    \boldsymbol{t^u} = (t^u_2 - t^u_1, \ldots, t^u_J - t^u_{J - 1})^\top \in \R^{J - 1}.
    \end{array}
  \right. 
\end{equation*}

...
\begin{tabular}{ccccc}
\toprule
Description & $\varphi_a(t, \beta_k^l, b^l)$ & $\imath_a$ & Reference \\
\midrule
...
\end{algorithm}
The convergence condition in Algorithm~\ref{alg:QNMCEM} is cv$(\Delta_{\text{rel}}^{(w+1)}) >$ cv$(\Delta_{\text{rel}}^{(w)})$, where cv$(\Delta_{\text{rel}}^{(w+1)})$ represents the variation at the $(w + 1)$-th iteration computed as
\begin{equation*}
...
\end{equation*}
where sd(.) and mean(.) are the empirical standard deviation and mean functions respectively, and relative difference $\Delta_{\text{rel}}^{(w+1)}$ is given by 
\begin{equation*}

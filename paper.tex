...
\begin{align}
  \label{eq:b_density}
  b_i | y_i, G_i=k; \theta &\sim \cN\Big((V_i^\top\Sigma_i^{-1}V_i + D^{-1})^{-1}V_i^T\Sigma_i^{-1}(y_i - U_i\beta_k), (V_i^\top\Sigma_i^{-1}V_i + D^{-1})^{-1}\Big) \nonumber \\
  y_i | G_i = k; \theta &\sim \cN\Big(U_i\beta_k, V_iDV_i^\top + \Sigma_i\Big)
\end{align}
...
Compute $C_i^{(w)} \in \R^{r \times r}$ such that $C_i^{(w)}{C_i^{(w)}}^\top = (V_i^\top{\Sigma_i^{(w)}}^{-1}V_i + {D^{(w)}}^{-1})^{-1}$ \tcp{Cholesky decomposition}

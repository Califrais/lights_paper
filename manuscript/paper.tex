...
\paragraph{Procedure.}
Let us describe here the procedure we follow to evaluate model performances on simulated data in Section~\ref{sec:simulation study} and on real data in Section~\ref{sec:application}.
\begin{algorithm}[!htb]
\DontPrintSemicolon
\caption{Procedure followed to assess model performances in our real-time prediction paradigm.}
...
\For{$\breve{r} = 1,\ldots,\breve{R}$}
{
  ...
  \tcc{For simplicity, we do not detail the cross-validation procedure here}
  $\hat \theta \leftarrow$ \texttt{model.fit}$(\cD_{n_{train}})$ \\
...
\textbf{Return:} {$\big[\hat \cR_{i,\breve{r}}\big]_{\substack{i=1,\ldots,n_{test} \\ \breve{r}=1,\ldots,\breve{R} \ \ \ }}\ , \ \big[\texttt{time}_{i,\breve{r}}\big]_{\substack{i=1,\ldots,n_{test} \\ \breve{r}=1,\ldots,\breve{R} \ \ \ }}$}
\end{algorithm}
...
tirage sur le test set des timax avec loi Beta
considérer 2 modèles avec 2 markers pour comparaison
dire ici qu'on tronque $y$ à $t^{max}$ et le noter $|t^{max}$ 
dire qu'on follow cette procedure sur donnees simulees et sur donnees reelles (mettre un if simulated-data : then simulate and data has b index)

...
This paper introduces a prognostic method called \textit{lights} to deal with the problem of joint modeling of longitudinal data and censored durations, where a large number of both longitudinal and time-independent features are available. Yet there is no standard model so far to learn from such high-dimensional multivariate longitudinal data in a survival analysis setting.
Shared association features are learned from the longitudinal processes and included as potential risk factor in a group-specific Cox model. Appropriate penalties are then used during inference to allow flexibility in modeling the dependency between the longitudinal features and the event time.
...
Inference is achieved using an efficient and novel proximal Quasi-Newton Monte Carlo Expectation Maximization algorithm (prox-QNMCEM).
...
\begin{equation}
  ...
  \lambda(t|\cM_k(t), G = k) = \lambda_0(t) \exp \Big\{\sum_{l=1}^L \sum_{a=1}^\cA {\gamma_{k,a}^l}^\top \Psi_a^l \Big\},
\end{equation}
where $\lambda_0(\cdot)$ is an unspecified baseline hazard function.

For each $l$-th longitudinal outcome, we consider $\cA \in \N_+$ known features $\Psi_a$ defining a shared association with $\gamma_{k,a}^l \in \R^{\cA}$ the corresponding joint association parameters.  These features are extracted from $Y$ by the \texttt{Python} library \texttt{tsfresh}. This can be viewed as a generalization of SREMs~\citep{rizopoulos2010jm}. Let us finally denote
...
\begin{equation}
  ...
  \lambda(t|\Psi, G = k) = \lambda_0(t) \exp \big\{\gamma_k^\top \Psi \big\}.
\end{equation}
...
\begin{assumption}
\label{assumption1}
We suppose that the random effects are conditional independent of the group membership on the association features, and that the latter remain independent conditional on the observed data (namely $T$, $\Delta$ and $Y$).
\end{assumption}
...
\tikzstyle{box}=[rectangle, draw=black!100]
  ...
  \node[main, fill = white!100] (b) [above=of mu] {\Large$\bm{b_i^l}$};
  \node[var, fill = white!100] (phi) [above=of Y] {\Large$\bm{\phi_l}$};
  \node[var, fill = white!100] (Sigma) [left=of b] {\Large$\bm{D}$};
  \node[main, fill = black!30] (W) [below=3cm of Y] {\Large$\bm{\Psi_i}$};
  ...
  \node[rectangle, inner sep=4mm, fit= (beta) (mu) (b) (Y) (b) (Sigma) (phi),ultra thick] at (8, 4.1) {\Large$\bm{l = 1...L}$};
...
\[ \cD_n = \big\{ (x_1, y_1^1, \ldots, y_1^L, t_1, \delta_1, \Psi_1), \ldots, (x_n, y_n^1, \ldots, y_n^L, t_n, \delta_n, \Psi_n) \big\} \]
...
\begin{assumption}
\label{indep-hyp-3}
Assume that the random effects vector $b_i$, the group membership account for the association between the longitudinal and event outcomes and the association features $\Psi_i$, that is
\begin{equation}
  \label{eq:ind-hyp}
  f(t_i, \delta_i, y_i | b_i, G_i = k, \Psi_i ; \theta) = f(t_i, \delta_i| G_i = k, \Psi_i ; \theta) f(y_i | b_i, G_i = k ; \theta)
\end{equation}
for all $i=1, \ldots, n$.
\end{assumption}
...
\begin{align}
	...
	&= n^{-1} \sum_{i=1}^n \log \sum_{k=0}^{K-1} \pi_{\xi_k}(x_i) \int_{\R^r} f(t_i, \delta_i| b_i, G_i = k, \Psi_i ; \theta) f(y_i | b_i, G_i = k, \Psi_i; \theta) f(b_i ; \theta) \dd b_i \\
  &= n^{-1} \sum_{i=1}^n \log \sum_{k=0}^{K-1} \pi_{\xi_k}(x_i) f(t_i, \delta_i| G_i = k, \Psi_i ; \theta) f(y_i | G_i = k; \theta) \dd b_i,
\end{align}
where 
\[f(t_i, \delta_i| G_i = k, \Psi_i ; \theta) = \big[\lambda(t_i|G_i = k, \Psi_i)\big]^{\delta_i} \exp \Big\{-\int_0^{t_i} \lambda(s|G_i = k, \Psi_i) \dd s \Big\}. \]
...
\begin{align*}
  ...
  & \quad + \delta_i \Big(\log \lambda_0(t_i) + \gamma_k^\top \Psi_i \Big) - \exp \big\{ \gamma_k^\top \Psi_i \big\} \int_0^{t_i} \lambda_0(s) \dd s \\
...
\begin{equation}
  ...
  \hat \Lambda_{ik,\tilde g}^{\theta^{(w)}} = \dfrac{1}{2N} \sum_{\breve{b} \in S_{ik}^{(w)}} \tilde g(\breve{b}) f(t_i, \delta_i, y_i | \breve{b}, G_i = k ; \theta^{(w)}).
\end{equation}
...
\begin{align*}
  Q^{(w)}_{n,k}(\gamma_k) = -n^{-1} \sum_{i=1}^n &\hat\pi_{ik}^{\theta^{(w)}} \Big[\delta_i \hat \E_{\theta^{(w)}}[ \gamma_k^\top \Psi_{k,i}(t_i) | t_i, \delta_i, y_i] \\
  & - \sum_{j=1}^J \lambda_0^{(w)}(t_j^u) \hat \E_{\theta^{(w)}}[\exp \big\{\gamma_k^\top \Psi_{k,i}(t_j^u)\big\} | t_i, \delta_i, y_i] \ind{\{t_j^u \leq t_i\}} \Big]
\end{align*}
...
\begin{align}
  \label{eq:grad-Q}
  &\frac{\partial Q_{n,k}^{(w)}(\gamma_k)}{\partial \gamma_{k,a}^l} = -n^{-1} \sum_{i=1}^n \hat \pi_{ik}^{\theta^{(w)}} \Big[ \delta_i  \hat \E_{\theta^{(w)}}[ \varphi_{k,a}(t_i, b_i^l) | t_i, \delta_i, y_i] \\ \nonumber
   &\qquad\qquad - \sum_{j=1}^J \lambda_0^{(w)}(t_j^u) \hat \E_{\theta^{(w)}}[ \varphi_{k,a}(t_j^u, b_i^l) \exp \big\{\gamma_k^\top \Psi_{k,i}(t_j^u)\big\} | t_i, \delta_i, y_i] \ind{\{t_j^u \leq t_i\}} \Big]
\end{align}
...
\begin{equation}
  ...
  \lambda_0^{(w+1)}(t)= \dfrac{\sum_{i=1}^n \delta_i \ind{\{t=t_i\}}}{\sum_{i=1}^n \sum_{k=0}^{K-1} \hat \pi_{ik}^{\theta^{(w)}} \hat \E_{\theta^{(w)}}\Big[\exp \big\{{\gamma_k^{(w+1)}}^\top \Psi_{k,i}(t)\big\} | t_i, \delta_i, y_i \Big] \ind{\{t_i \geq t\}} },
\end{equation}
...
\end{equation}
for the choice of the association parameters, with $\cA = 3 + 3(1-r_s)/r_s$ so that $(i)$ there is a proportion $r_s$ of active longitudinal features among the $L$ (with the $\cS_k$), $(ii)$ we induce sparsity within each group adding a proportion of $(1-r_s)$ non-active association features ($a \geq 3$) in each group (similarly to~\eqref{eq:xi-sparse} for the time-independent features), and $(iii)$ the longitudinal effects are distinct for each subgroup $k=0, \ldots, K-1$ (thanks to the $\cS_k$ definition).
...
\begin{equation*}
  \left\{
    ...
    \tilde g^3_{s, k}&: b_i \mapsto \Psi_{k,i}(s), \\
    \tilde g^4_{s, \gamma_k}&: b_i \mapsto \exp \big\{\gamma_k^\top g^3_{s, k}(b_i) \big\}, \\
    \tilde g^5_{s, \gamma_k}&: b_i \mapsto g^3_{s, k}(b_i)^\top g^4_{s, k}(b_i).
    \end{array}
  \right. 
\end{equation*}
We also denote 
...
\end{bmatrix}
\in \R^{n \times J} \]
some matrices to compare each event times with the vector of distinct event times. Those matrices are widely used in practice to replace indicators. For instance, the quantities
\begin{align*}
f(t_i, \delta_i| \breve{b}, G_i = k ; \theta^{(w)}) = &\big(\lambda_0^{(w)}(t_i) g^4_{t_i, \gamma_k^{(w)}}(\breve{b}) \big)^{\delta_i} \\ 
&\times \exp \Big\{ - \sum_{j=1}^J \lambda_0^{(w)}(t_j^u) g^4_{t_j^u, \gamma_k^{(w)}}(\breve{b}) \ind{\{t_j^u \leq t_i\}} \Big\}
\end{align*}
for $i = 1, \ldots, n$ and $\breve{b} \in S^{(w)}$ can be represented in the following matrix 
...
\begin{equation*}
  \left\{
    \begin{array}{ll}
    \boldsymbol{g^4_{\gamma_k}} \in \R^{2N \times J} \text{ with } \big[\boldsymbol{g^4_{\gamma_k}}\big]_{i, j} = \tilde g^4_{t_j^u, \gamma_k^{(w)}}(\breve{b}_i) , \\

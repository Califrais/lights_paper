...
{ 
  \tcc{E-step}
  Compute $\hat \Lambda_{ik,\tilde g}^{\theta^{(w)}}$ using~\eqref{eq:Lambda_ik-b_i} for the required functions $\tilde g$\\
...
Then in practice, the convergence for Algorithm~\ref{alg:prox-QNEM} is declared when the condition
\[\Delta_{\text{rel}}^{(w+1)} < \epsilon_2\]
is satisfied for 3 consecutive iterations.
...
Let us derive here the explicit EM algorithm for the multivariate linear mixed model used to initialized the longitudinal parameters $\beta_k^{(0)}$, $D^{(0)}$ and $\phi^{(0)}$ in the prox-QNEM algorithm in Section~\ref{sec:prox-QNEM}, acting as if there is no subgroup ($\beta_0^{(0)} = \cdots = \beta_{K-1}^{(0)}$).

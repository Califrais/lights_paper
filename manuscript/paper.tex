...
with
\begin{align}
  \Lambda_{ik,\tilde g}^{\theta^{(w)}} &= \int_{\R^r} \tilde g(b_i) f(t_i, \delta_i | b_i, G_i = k, \Psi_i ; \theta^{(w)}) f(y_i | b_i, G_i = k, \Psi_i ; \theta^{(w)}) f(b_i | G_i = k, \Psi_i; \theta^{(w)}) \dd b_i \\
  \label{eq:Lambda_ik-b_i}
  &=  f(t_i, \delta_i | G_i = k, \Psi_i ; \theta^{(w)}) f(y_i | G_i = k ; \theta^{(w)}) \int_{\R^r} \tilde g(b_i) f(b_i | y_i, G_i = k ; \theta^{(w)}) \dd b_i,
\end{align}
where
\begin{align*}
